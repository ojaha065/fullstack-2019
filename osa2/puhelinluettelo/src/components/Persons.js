...
const Persons = (props) => {
    ...
    const rows = filtered.map((person) => {
        return (<li key={person.number}>{person.name} {person.number} <button id={person.id} type="button" onClick={(e) => props.removeNumber(Number(e.target.id))}>remove</button></li>);
    });
