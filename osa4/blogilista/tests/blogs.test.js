...
describe("POST /api/blogs",() => {
    ...
    test("If number of likes is not given, likes should be set to zero",async () => {
        let copy = {...newBlog};
        delete copy.likes;
        await api.post("/api/blogs").send(copy).expect(201);
    ...
    test("New blogs without title or url should not be accepted",async () => {
        let copy = {...newBlog};
        delete copy.title;
        await api.post("/api/blogs").send(copy).expect(400);
...
describe("DELETE /api/blogs",() => {
    test("API should return the right number of blogs after DELETE",async () => {
        ...
        id = id.body[0].id;
        await api.delete(`/api/blogs/${id}`).expect(205);
    ...
    test("Wrong or missing ID returns 404",async () => {
        await api.delete("/api/blogs/LoremIpsum").expect(404);
